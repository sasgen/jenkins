...
// 上传文件
def upload(endpointUlr,credentials,bucket,file,path){
    withAWS(endpointUrl:endpointUlr,credentials:credentials){
        s3Upload(file:file, bucket:bucket, path:path)
    }
}

// 下载文件
def download(endpointUlr,credentials,bucket,file,path){
    withAWS(endpointUrl:endpointUlr,credentials:credentials){
